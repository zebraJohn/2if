...
class Times2DBackbone(nn.Module):
    def __init__(self, configs, **kwargs):
        ...
        # 周期集合可配置（组合模型按 seq_len 设定，如 seq_len=96 -> [48,24,12,6]）
    ...
    def _period_features(self, x):  # x: [B, N, T]（已归一化），返回各尺度周期特征 list
        x = x.to(self.device)
        B, N, T = x.size()
        res = []
        for i, (period, (kernel_height, kernel_width)) in enumerate(zip(self.period_list, self.kernel_list)):
            ...
            out = rearrange(out, '(b n) d p -> b n d p', b=x.size(0))  # [B, N, dim_list[i], tokens_list[i]]
            glo = self.backbone[i](out)                                # [B, N, T]
            res.append(glo)
        return res

    def forward_features(self, x):  # x: [B, N, T]（外部已归一化），返回融合多尺度周期特征 [B, N, T]
        x = x.to(self.device)
        res = self._period_features(x)
        z = torch.stack(res, dim=-1).mean(dim=-1)   # [B, N, T]
        return z
